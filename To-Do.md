- [ ] troubleshoot Beckham law notification https://maps.app.goo.gl/XeSN897uStjpovPc7
- [ ] black marker shoe
- [ ] plan food for the week and trip
- [ ] packing list 

## Travel & Logistics

- [ ] Check what metro segments will be closed
- [ ] Oncall Exemption
- [ ] Going to the beach directly from office:
  - [ ] Keep shorts in backpack
  - [ ] Potentially pack bocata or look for pizza/bar nearby
- [ ] Hat
- [ ] Scuffers placa Catalunya
- [ ] Bathing suit or short seam wide shorts
- [ ] Wired headphones
- [ ] Rental services Lauterbrunnen  
  https://www.balmers.com/where-to-rent-gear-in-interlaken/
- [ ] Hiking gear (boots), check shops around for pants
- [ ] Plan what food to pack for Switzerland:
  - [ ] Tuna
  - [ ] Canned beans
  - [ ] Cured meats
  - [ ] Hard cheeses
  - [ ] Pre-packaged bread (the one that lasts forever) or tortillas
  - [ ] Oats
- [ ] Zurich drop off ok — how about Zurich pick up too? Nope outbound is on me
- [ ] Check Zurich Airbnb
- [ ] Plan holidays year
- [ ] Reschedule or give up on May 15th flight
- [ ] Reschedule May flights — sort out period + how many days in August
- [x] Book Madrid trains
- [x] Book Aveiro flights + trains
- [x] Book Lisbon Airbnb
- [x] Eventually look for Dublin accommodation
- [x] Book Ancona Bologna + Bologna Ancona trains
- [x] Airbnb check in

## Content Creation / LifeMaxxing

- [ ] Highfield AI and other editing tools saved on TikTok
- [ ] Increase surface area for luck — Sidequest/LifeMaxxing
- [ ] Content strategy / life planning
- [ ] Structured content creation
- [ ] Start recording yourself/content creation streak (100 day challenge-ish)
- [ ] Find out how/where to sell protein bars
- [ ] Stary reading
- [x] Good morning in each language
- [ ] Delve into lucid dreaming science
- [ ] Delve into the language of flowers

## Spain Bureaucracy / Admin

- [ ] File tax return
- [ ] Upload all travel/move related expenses to Graebel
- [ ] Submit report:
  - [ ] Upload Metro card receipt
  - [ ] Upload airport ticket as receipt
- [ ] Spanish phone number
- [ ] All bureaucracy related stuff Spain
- [ ] Apply for Beckham's law within 6 months (Form 149)
- [ ] Statement from Tax Authority in country of residence confirming total income from all other sources (01/01/2024 - 31/12/2024)
- [ ] Check Revenue docs
- [ ] Check Vialto docs
- [ ] Call Revenue to ask to be tax resident and receive tax credit
- [ ] Upload IBAN on fisco website for IVA refund
- [x] Check lease contract

## Career / Amazon / Work

- [ ] Start Leetcoding
- [ ] Check extra benefits  
  https://extrasforamazon.com/welcome/register
- [ ] Slack housing channel
- [x] Ask team about Simone's relocation and financial advice
- [x] Find out whether Revolut investing is worth it + how
- [x] Reach out to Simone for relocating to Spain advice
- [x] Questions for Rafa:
  - [x] How to be successful in this team
  - [x] How to help with design processes while still learning the domain
  - [x] How to contribute beyond implementations
- [x] Need to submit reports within Friday because card will be closed
- [x] Time block more — context switching kills productivity
- [x] Salary negotiation
- [x] Ask team about financial advice
- [x] Check in with relocation agency
- [x] Return laptop
- [x] Return laptop on Feb 26th

## Software / Projects

- [x] Describe changesets
- [x] If enough time:
  - [x] Receive spending summary / over-budget alerts
  - [x] Add in ReFunct
  - [x] Define use case
  - [x] Create Use Case Diagram
- [x] Add delete for transactions
- [x] Add budgets/categories
- [x] Proper filtering in Flutter
- [x] Possibly right colour scheme
- [x] Technical error:
  - [x] `"An unexpected error occurred", message: "Object of type Decimal is not JSON serializable"`
- [x] Redeploy chat app stack to eu-west-2 without overwriting stuff
- [x] Lambda Monolith → Microservices  
  https://docs.aws.amazon.com/it_it/lambda/latest/dg/concepts-event-driven-architectures.html
- [x] Ask Andrei about washing machine/dryer
- [x] Ask Storti:
  - [x] Jetpack Compose suitability
  - [x] Functional requirements
  - [x] Firebase file upload restrictions
  - [x] Flutter version of app

## Investing / Finance

- [ ] Start investing
- [ ] Ask team about Revolut investing
- [ ] Puneet money back
- [ ] Ask Raffaele Cocco money again
- [ ] Reach out to Raffaele Cocco for 20€
- [x] Latvian girl pizza money

## Social / Relationships

- [ ] You look like a chill guy + the guy who needs sun
- [ ] Leo and I accidentally went to the same restaurant in Shanghai
- [ ] Talked with Maria about Australia
- [ ] Just let Elisa know I am alive — find out what she's up to
- [ ] Send stuff to Leo's family:
  - [ ] Alfonsina
  - [ ] Giuseppe
  - [ ] Francesca (zia)
  - [ ] Dog name Pita
  - [ ] Something thumb 👍 related for Alfonsina
- [ ] Porchetta related stuff for Dawid
- [ ] Knitting stuff for Iulia
- [ ] Schedule coffee with Fachtna
- [ ] Shoot message to Lara for coffee rave
- [ ] Send messages/referrals for Leo
- [x] Elea birthday message/letter
- [x] Reply to Elsi for wisdom teeth removal
- [x] Reach out to Keith Howard
- [x] Reach out to Nadya and Amri about apartment free from November
- [x] Casually ask mom about songs she likes like "Shiny Happy People"
- [ ] Tell parents you love them

## Fitness / Health / Food

- [ ] Eating more carbs
- [ ] Balance kettlebells, strength, running, calisthenics and beach
- [ ] Gym membership
- [x] Gym in shorts + tote bag routine:
  - [ ] Figure out running/ski/heavy lifts
  - [ ] Change so outfit ready
  - [ ] Come back
  - [ ] Hang white towel
  - [ ] Move shorts and black towel to backpack
  - [ ] Eat mozzarella
  - [ ] Pack eggs
- [ ] Hi-pro Basque cheesecake
- [ ] Pasta zozzona
- [ ] Your bucket list should be eating cottage cheese from each country you visit

## Packing / Clothing

- [ ] Parachute pants
- [ ] Beige cargo
- [ ] Guitalele
- [ ] 2 Nike shorts
- [ ] Black short socks
- [ ] White long socks
- [ ] White shirts
- [ ] Sweaters
- [ ] Running vest
- [ ] Kway
- [ ] Protein bars
- [ ] Beauty case
- [ ] Loop bands
- [ ] Fitness land towel
- [ ] Big blue towel
- [ ] Small bottle with sea salt spray
- [ ] Gym t-shirts
- [x] Hide Elf bars properly
- [x] Replace phone screen
- [x] Check Carhartt aviation pants physically
- [ ] Gigli life changing shoes
- [ ] First Ascent  
  https://spain.firstascent.io/

## Food / Places / Experiences

- [ ] Cascate delle cento fonti
- [ ] Bologna getaway
- [x] Water parks
- [x] Palacio Chiado
- [ ] Sagrada 140th anniversary band concert
- [ ] Tour de France
- [ ] Petardos
- [ ] Golden flowers
- [ ] Golden flowers on the ground
- [ ] Acqua ossigenata worked for stains
- [ ] I met Eduardo IRL
- [ ] Park made out of hated bushes in backyard
- [ ] Mental sanity music girl Tesco

## Random Notes / Stories To Tell

- [ ] Tell about white hair
- [ ] Tell about orange bottle
- [ ] First new glass bottle — perfect timing
- [ ] Another bottle + smoothie + caricature
- [ ] Tell about Eduardo in person
- [ ] Tell about golden flowers
- [ ] Tell about Tour de France
- [ ] Fire alarms
- [ ] Tell about park made out of hated bushes
- [ ] Mention walking and fascia because of looking older